...
LIST_OF_INTERNAL_EXCEPTIONS = ["UncheckedIOException", "ArithmeticException", "ArrayIndexOutOfBoundsException", "ArrayStoreException", "ClassCastException", "IllegalArgumentException", "IllegalMonitorStateException", "IllegalStateException", "IllegalThreadStateException", "IndexOutOfBoundsException", "NegativeArraySizeException", "NullPointerException", "NumberFormatException", "SecurityException", "UnsupportedOperationException"]
...
def get_new_code_line_after_add_suitable_method_signature(pointer, method_desc, code_line, possible_matches):
	...
	ptr = len(pointer) - 2
	while(not (code_line[ptr:].startswith(method_name) or code_line[ptr:].startswith("this") or code_line[ptr:].startswith("super"))):
		ptr += 1
		if(ptr > len(code_line)):
			return code_line
...
class Exception:
	...
	def analyse_exception(self):
		...
		if("is public, should be declared in a file named" in self.exception_desc):
			l_code[self.line_number] = PUBLIC_CLASS_STRING + "\n" + l_code[self.line_number].replace("public class", "class")
		...
		if(self.exception_desc.startswith("incompatible types: ") and "cannot be converted to" in self.exception_desc):
			...
			if(target_type == DUMMY_RETURN_TYPE):
				...
				if(len(var_name) < 2):
					l_code[self.line_number] = get_new_code_line_after_add_data_type(self.l_e[2], VALID_CHARS, target_type, l_code[self.line_number], "left")
					self.add_to_dont_touch_list()
					return
				var_name = var_name[1]
			...
			if(target_type == "Throwable"):
				...
				new_line = curr_line[:ptr_pos+1] + target_type + curr_line[ptr_pos+1:colon_pos+1] + target_type +  curr_line[colon_pos+1:]
				l_code[self.line_number] = new_line
			else:
				l_code[self.line_number] = get_new_code_line_after_add_data_type(self.l_e[2], VALID_CHARS, target_type, l_code[self.line_number], direction)
			self.add_to_dont_touch_list()
			return
		...
		if("is never thrown in body of corresponding try statement" in self.exception_desc):
			...
			l_code[self.line_number] = l_code[self.line_number].replace(exception_name, LIST_OF_INTERNAL_EXCEPTIONS[exception_list_index + 1])
		...
		if("exception" in self.exception_desc and " has already been caught" in self.exception_desc):
			...
			if(exception_name in LIST_OF_INTERNAL_EXCEPTIONS):
				new_exception = LIST_OF_INTERNAL_EXCEPTIONS[(LIST_OF_INTERNAL_EXCEPTIONS.index(exception_name) + 1) % len(LIST_OF_INTERNAL_EXCEPTIONS)]
			else:
		...
		if(self.exception_desc.startswith("no suitable method found for") or self.exception_desc.startswith("no suitable constructor found for")):
			...
			l_code[self.line_number] = get_new_code_line_after_add_suitable_method_signature(self.l_e[2], method_desc, l_code[self.line_number], possible_matches)
			self.add_to_dont_touch_list()
			return
		...
		if(self.exception_desc.startswith("array required, but ")):
			...
			if(l_code[self.line_number-1].strip() == creation_str):
				l_code[self.line_number-1] = DUMMY_RETURN_TYPE + "[] " + host_var + " = new " + DUMMY_RETURN_TYPE + "[5];"
		...
		if(self.exception_desc.startswith("bad operand type ") and " for unary operator " in self.exception_desc):
			...
			if(self.exception_desc.endswith("'!'")): #to go right
				ptr = len(self.l_e[2])
				l_code[self.line_number] = l_code[self.line_number][:ptr] + "(Boolean)(Object)" + l_code[self.line_number][ptr:]
				self.add_to_dont_touch_list()
				return
		...
		if(self.exception_desc.startswith("non-static variable") and " cannot be referenced from a static context" in self.exception_desc):
		...
		if(self.exception_desc.startswith("non-static method") and " cannot be referenced from a static context" in self.exception_desc):
		...
		if(self.exception_desc.startswith("type ") and "does not take parameters" in self.exception_desc):
			ptr = len(self.l_e[2]) - 2
			l_code[self.line_number] =  l_code[self.line_number][:ptr] + re.sub(r'\<[^>]*\>', '', l_code[self.line_number][ptr:])
			self.add_to_dont_touch_list()
			return
		...
		if(self.exception_desc.startswith("cannot find symbol")):
			...
			if(not ls_does_contain(self.l_e, "location")): # no location line
				...
				if(symbol.startswith("class ")):
					...
					if(class_name.endswith("Exception") or class_name.startswith("error") or ("catch" in l_code[self.line_number])):
